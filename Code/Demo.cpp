...
void Demo::execute_init_cmd()
{
	...
	// Now that we've gotten past that line ^^ that 
	// means the fence must be open, which means the 
	// GPU must be finished executing the 
	// initalization command buffer

	// We won't need to use this command buffer ever again,
	// so we can destroy the command buffer, as well as the
	// initFence

	// reset the command buffer
	vkFreeCommandBuffers(device, cmd_pool, 1, &initCmd);

	// Destroy the initFence, because we won't need
	// it anymore, now that we don't have initCmd
...
void Demo::prepare()
{
	...
	if (firstInit)
	{
		...
		// Insdie Square.frag, you will see the
